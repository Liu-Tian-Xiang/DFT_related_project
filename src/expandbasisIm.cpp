...
double basisIminner(double r,void *params){
    //x[0-L]<->r[0-R]
    double coef,x;
    basisImparameter *para=(basisImparameter *)params;

    x=para->x;
    x=x*L_sys*2;
    r=r*R_sys;

    return (*para->pV)(r,x,para->ptr)*R_sys*(L_sys*2)*expand(r,x,para->m,para->n_2,para->l_2,"Im");
}
double basisImouter(double x,void *params){
...
double expandbasisIm(int n_1,int l_1,int n_2,int l_2,int m,Fun/*cPtr*/ pV,spline_space *who){
    ...
    basisImparameter para={n_1,l_1,n_2,l_2,m,&pV,who,0.0};
    gsl_function f;
    gsl_integration_workspace *work;
    f.function=basisImouter;
    f.params=&para;
    double err,result;
