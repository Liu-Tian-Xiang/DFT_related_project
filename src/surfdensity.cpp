...
for(int th=0;th<=2*PI*hth;th++){
		...
		//printf("%s(%d,%d,%d)=%1.4e;\n",prename,r+1,th+1,x+1,para->rho(r/h,x/h)); 
		cout<<prename<<"("<<floor(R_sys*h+r*cos(th/hth))+1<<","<<floor(R_sys*h+r*sin(th/hth))+1<<","<<x+1<<")="<<para->rho(r/h,x/h)<<";"<<endl;
		//printf("%s(%1.5e,%1.5e)=%1.4e;\n",prename,r*1.0/h,x*1.0/h,para->rho(r*1.0/h,x*1.0/h)); 
