...
void Wave_function::stRHO(){
	...
	for(int r = 0; r <R_floor; r++){
		...
		gsl_spline2d_set(spline_, spline_values_, r, z ,0.3*rho(r*h,z*h)+0.7*interpV(r*h,z*h,&Mix));
	    }
	}
    }else{
	for(int r = 0; r <R_floor; r++){
	    for(int z = 0 ; z <L_floor; z++){
		gsl_spline2d_set(spline_, spline_values_, r, z ,rho(r*h,z*h));
	    }
	}
    }
